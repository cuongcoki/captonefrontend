...
// ** import ICON
import { Eye, ImageUp, Minus, PackagePlus, PencilLine, Trash2 } from "lucide-react";
...
export const SetUpdateForm: React.FC<SetID> = ({ setId }) => {
    ...
    const [addProducts, setAddProducts] = useState<{ productId: string, quantity: number }[]>([]);
    ...
    const handleChangeUpdate = (productId: string, newQuantity: number) => {
        const existingProductIndex = updateProducts.findIndex(product => product.productId === productId);
        ...
        if (existingProductIndex !== -1) {
            ...
            updatedProductsRequest = [...updateProducts, { productId, quantity: newQuantity }];
        }
    ...
    const handleAddProducts = (product: any) => {
        ...
        const existingDetailProduct = getDetailsPro.find((item) => item.id === product.id);
        ...
        const existingProduct = productsRequest.find((item) => item.productId === product.id);
        ...
        if (existingProduct) {
            ...
            setProductsRequest([...productsRequest, { productId: product.id, quantity: 1 }]);
        }
    };
    ...
    const handleDeleteProducts = (productID: string) => {
        ...
        const updatedProductsRequest = updateProducts.filter((item) => item.productId !== productID);
        setUpdateProducts(updatedProductsRequest);
        const updateProUpdate = getDetailsProUpdate.filter((item) => item.productId !== productID);
    ...
    const handleMinusProducts = (productId: string) => {
        ...
        const updatedProductsRequest = productsRequest.filter((product) => product.productId !== productId);
        setProductsRequest(updatedProductsRequest);

        // Nếu sản phẩm có trong danh sách updateProducts, loại bỏ nó
        const updatedUpdateProducts = updateProducts.filter((product) => product.productId !== productId);
    ...
    const handleChange = (productId: string, newQuantity: number) => {
        const existingProductIndex = productsRequest.findIndex(product => product.productId === productId);
        ...
        if (existingProductIndex !== -1) {
            ...
            // Nếu sản phẩm chưa tồn tại trong danh sách, thêm sản phẩm vào danh sách với số lượng mới
            updatedProductsRequest = [...productsRequest, { productId, quantity: newQuantity }];
        }
                                    ...
                                    <form onSubmit={form.handleSubmit(onSubmit)} className='flex flex-col gap-6'>
                                        ...
                                        <div className="grid gap-4 md:grid-cols-[1fr_250px] lg:grid-cols-3 lg:gap-8">
                                            ...
                                            <div className="grid auto-rows-max items-start gap-4 lg:col-span-2 lg:gap-8 ">
                                                <Card>
                                                    <CardHeader>
                                                        <CardTitle className="text-lg">Chi tiết bộ sản phẩm</CardTitle>
                                                    </CardHeader>
                                            ...
                                            <div className="grid auto-rows-max items-start gap-4 lg:gap-8">
                                                <Card className="overflow-hidden" x-chunk="dashboard-07-chunk-4">
                                                    <CardHeader>
                                                        <CardTitle className="text-lg">Ảnh đặt sản phẩm</CardTitle>
                                                    </CardHeader>
                                                    <CardContent>
                                                        ...
                                                        <div className='flex gap-4 justify-center items-center p-4'>
                                                            <Trash2 className="h-6 w-6 text-red-600" onClick={handleDeleteImage} />
                                        ...
                                        <div className="grid auto-rows-max items-start gap-4 lg:col-span-2 lg:gap-8">
                                            <div className="grid auto-rows-max items-start gap-4 lg:col-span-2 lg:gap-8">
                                                <Card >
                                                    <CardHeader>
                                                        <CardTitle className="text-lg">Chi tiết sản phẩm trong bộ</CardTitle>
                                                                    ...
                                                                    {getDetailsProUpdate.map((product, index) => (
                                                                        <div className="flex justify-between items-center py-4" key={index}>
                                                                            ...
                                                                            <div className="flex  gap-4" >
                                                                                <Image alt="ảnh mẫu" className="w-[100px] h-[100px] object-cover" width={900} height={900} src={product?.product.imageResponses[0].imageUrl} />
                                                                                <div className="font-medium dark:text-white">
                                                                                    <div><b>Tên: </b>{product.product.name}</div>
                                                                                    <div className="text-sm text-gray-500 dark:text-gray-400"><b>Code: </b>{product.product.code}</div>
                                                                                    <div className="text-sm text-gray-500 dark:text-gray-400"><i>{product.product.description}</i></div>
                                                                                </div>
                                                                            </div>
                                                                            ...
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => handleChangeUpdate(product.productId, parseInt(e.target.value))}
                                                                                className="w-16 text-center outline-none"
                                                                            />
                                                                            <Button variant="outline" size="icon" onClick={() => handleDeleteProducts(product.productId)}>
