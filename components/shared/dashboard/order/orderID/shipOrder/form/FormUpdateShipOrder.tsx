...
export const FormUpdateShipOrder: React.FC<FormUpdateShipOrderProps> = ({
  shipOrderId,
  orderId,
}) => {
  // ** state for dialog
  ...
  const onSubmit = (data: z.infer<typeof ShipOrderSchema>) => {
    ...
    setLoading(true);
    shipOrderApi
      .updateShipOrder(requestBody, shipOrderId.shipOrderId)
      .then(({ data }) => {
        if (data.isSuccess) {
          ForceRender();
          setOpen(false);
  ...
  const [originalShipOrderDetailRequests, setOriginalShipOrderDetailRequests] =
    useState<any[]>([]);
  ...
  const handleOffDialog = () => {
    ...
    const isFormChanged =
      JSON.stringify(shipOrderDetailRequests) ===
      JSON.stringify(originalShipOrderDetailRequests);

    if (!formState.isDirty && isFormChanged) {
      setOpen(false);
      setFetchTrigger((prev) => prev + 1);
    } else if (
      Array.isArray(originalShipOrderDetailRequests) &&
      originalShipOrderDetailRequests.length === 0
    ) {
      setOpen(false);
      setFetchTrigger((prev) => prev + 1);
    } else {
                  ...
                  <Form {...form}>
                    ...
                    >
                      ...
                      <Button
                        ...
                        disabled={loading}
                      >
                        {loading ? "Loading..." : "Chỉnh sửa đơn hàng"}
                      </Button>
                    </form>
                  </Form>
                </div>
