...
export const ShipmentID: React.FC<ShipmentIDProps> = ({ shipmentIDDes }) => {
    ...
    useEffect(() => {
        const fetchDataOrderId = () => {
            setLoading(true)
            shipmentApi.getShipmentID(shipmentIDDes)
                .then(({ data }) => {
                                    ...
                                    >
                                        <CardHeader className="flex flex-row items-start bg-muted/50">
                                            <div className="grid gap-0.5">
                                                ...
                                                <CardDescription>Ngày: {data?.shipDate ? formatDate(data.shipDate) : 'N/A'}</CardDescription>
                                            </div>
                                        ...
                                        <CardContent className="p-6 text-sm">
                                                            ...
                                                            <li className="p-2 shadow-sm hover:bg-gray-100/90 cursor-pointer" key={index}>
                                                                <span className="text-muted-foreground flex justify-between items-center mb-1">
                                                                    <span className="w-[50px] h-[50px] shadow-md rounded-md">
                                                                        <Link href={`/dashboard/products/product/${item.product.id}`}> <ImageIconShipment dataImage={item.product?.imageResponses} /> </Link>
                                                                    </span>
                                                                ...
                                                                <div className="flex gap-2">
                                                                    <span className="text-muted-foreground flex justify-between items-center ">{item.phase.name}</span>
                                                                    -
                                                                    <span className="text-muted-foreground flex justify-between items-center ">{item.phase.description}</span>
                                                                    -
                                                                    <span className="text-muted-foreground flex justify-between items-center ">{item.productPhaseTypeDescription}</span>
                                                                </div>
                                                            </li>
                                                        ))
                                                    }
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="grid gap-3">
                                                    <div className="font-semibold">Thông tin nhận viên vận chuyển</div>
                                                    <address className="grid gap-0.5 not-italic text-muted-foreground">
                                                        <Link href={`/profile/${data?.shipper.id}`} className="hover:underline">
                                                            <span>{data?.shipper.lastName} {data?.shipper.firstName} - {data?.shipper.companyName}</span>
                                                        </Link>
                                            ...
                                            <Separator className="my-4" />
                                            <div className="grid gap-3">
                                                <div className="font-semibold">Thông tin công ty khách hàng</div>
                                                <dl className="grid gap-3">
                                                    <div className="flex items-center justify-between">
                                                        <dt className="text-muted-foreground">Tên chủ tịch</dt>
